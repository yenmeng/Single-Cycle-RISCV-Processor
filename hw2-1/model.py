...
class Decoder(nn.Module):
	...
	def forward(self, x, c):
        #for interpolation
		'''c_0 = Variable(torch.from_numpy(np.array([0]))).cuda()
		c_1 = Variable(torch.from_numpy(np.array([1]))).cuda()'''
		# conv layer
		'''out = self.conv_block(x, [self.conv1, self.conv2], self.ins_norm1, (self.emb1(c_0)+self.emb1(c_1))/2, res=True )
		out = self.conv_block(out, [self.conv3, self.conv4], self.ins_norm2, (self.emb2(c_0)+self.emb2(c_1))/2, res=True)
		out = self.conv_block(out, [self.conv5, self.conv6], self.ins_norm3, (self.emb3(c_0)+self.emb3(c_1))/2, res=True)'''
		...
		# dense layer
		'''out = self.dense_block(out, (self.emb4(c_0)+self.emb4(c_1))/2, [self.dense1, self.dense2], self.ins_norm4, res=True)
		out = self.dense_block(out, (self.emb4(c_0)+self.emb4(c_1))/2, [self.dense3, self.dense4], self.ins_norm5, res=True)
		emb = (self.emb5(c_0)+self.emb5(c_1))/2'''
		...
		'''out = append_emb((self.emb5(c_0)+self.emb5(c_1))/2,out.size(2), out)'''
